...
const Article = ({
  ...
  const suffix =
    sourcesuffix && mediumsuffix ? `?${sourcesuffix}${mediumsuffix}` : ''
          ...
          <a href={url + suffix} target="_blank" rel="noopener noreferrer">
            <figure>
              <div className="imgwrap">
                <img onError={addDefaultSrc} src={img} alt={title} />
              </div>
              <figcaption>
                {icon && icon.length > 0 && (
                  <span className="icon">
                ...
                {tag && tag.length > 0 && <span className="tag">{tag}</span>}
                <h2>
                  {title && title.length > 0
                    ? title
                    : 'Nostrud aliquip tempor excepteur ut eiusmod cillum.'}
                </h2>
                <p>{excerpt}</p>
                {theme && theme === 'uno' && (
